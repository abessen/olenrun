...
def load_data(wb_file_path, rolling_window):
    ...
    while True:
        try:
            with pd.ExcelFile(wb_file_path, engine='openpyxl') as xls:
                df = pd.read_excel(
                    xls,
                    sheet_name='Sheet1',
                    skiprows=18,
...
rolling_window = st.sidebar.number_input("Enter Rolling Average Minutes", min_value=0, max_value=100, value=7, step=1, key='rolling_window')
...
def create_chart(df, y_axis_limit, tick_interval):
    ...
    color_scale = alt.Scale(
        ...
        range=['rgb(255, 0, 0)', 'rgb(0, 255, 0)', 'rgb(0, 0, 255)', 'rgb(255, 255, 0)', 'rgb(255, 0, 0)', 'rgb(0, 255, 0)', 'rgb(0, 0, 255)', 'rgb(255, 255, 0)',
               'rgb(255, 0, 0)', 'rgb(0, 255, 0)', 'rgb(0, 0, 255)', 'rgb(255, 255, 0)', 'rgb(0, 0, 255)']
    )
    for scale in y_select:
        bar_chart = alt.Chart(
            df[df['Scale'] == scale], height=500).mark_bar().encode(
